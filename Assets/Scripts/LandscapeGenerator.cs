...
public class LandscapeGenerator : MonoBehaviour
{
    ...
    public Vector3 groundScale = new Vector3(1, 1, 1);
    ...
    {
        int mulplier = 3;
        float waitRock = rockCount * 0.002f * mulplier;
        float waitTree = treeCount * 0.002f * mulplier;
        float waitGrass = grassCount * 0.002f * mulplier;
    ...
    public void GroundGenerator()
    {
        ...
        if (isRotate){
            randRotation = new Vector3(0, Random.Range(0, 360), 0);
        }
        Vector3 randPosition = initPosition;
        GameObject selectedGround = groundObjects[Random.Range(0, groundObjects.Length)];
        //Debug.Log(selectedGround.transform.localScale);
        selectedGround.transform.localScale = groundScale;
    ...
    public IEnumerator RockGenerator()
    {
        ...
        {
            Vector3 randPosition = new Vector3(Random.Range(-gameSize * groundScale.x / 2, gameSize * groundScale.x / 2), 50, Random.Range(-gameSize * groundScale.z / 2, gameSize * groundScale.z / 2));
            randPosition += initPosition;
    ...
    public IEnumerator GrassGenerator()
    {
        ...
        for (int i = 0; i < grassCount; i++)
        {
            ...
            if (Physics.Raycast(randPosition, Vector3.down, out hit))
            {
                ...
                grassClone.transform.rotation *= Quaternion.Euler(1, Random.Range(0, 360), 1);
